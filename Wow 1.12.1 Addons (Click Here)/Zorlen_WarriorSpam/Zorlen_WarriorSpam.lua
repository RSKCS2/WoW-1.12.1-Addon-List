...
local MinimumRequiredZorlenBuildNumber = 678
...
function Zorlen_WarriorSpam_OnUpdate(arg1)
	if not Zorlen_WarriorSpam_isCurrentClassWarrior or (not this.WarriorSpamBerserkerStanceDelay_timer and not this.MockingBlowDelay_timer and not this.RefreshIcons_timer and not Zorlen_IsTimer("WarriorSpamEnteringWorld", nil, "InternalZorlenMiscTimer")) then
		this:Hide()
	end
	...
	if Zorlen_IsTimer("WarriorSpamEnteringWorld", nil, "InternalZorlenMiscTimer") then
		if Zorlen_GetTimer("WarriorSpamEnteringWorld", nil, "InternalZorlenMiscTimer") <= 0 then
			Zorlen_WarriorSpam_RegisterMacroIcons()
			if Zorlen_WarriorSpamMakeFirstMacros then
				Zorlen_WarriorSpam_MakeMacros(1, 1)
				Zorlen_WarriorSpamMakeFirstMacros = nil
			...
			Zorlen_ClearTimer("WarriorSpamEnteringWorld", nil, "InternalZorlenMiscTimer")
		end
	end
end
...
function Zorlen_WarriorSpam_OnEvent(event, arg1, arg2, arg3)
	...
	if (event == ("PLAYER_ENTERING_WORLD")) then
		...
		Zorlen_WarriorSpam_CheckVariables()
		Zorlen_SetTimer(2, "WarriorSpamEnteringWorld", nil, "InternalZorlenMiscTimer", 2, 1)
	...
	elseif Zorlen_WarriorSpam_isCurrentClassWarrior then
		...
		elseif (event == "UPDATE_BONUS_ACTIONBAR") then
			...
			if (Zorlen_WarriorSpam_CurrentStance == LOCALIZATION_ZORLEN_BerserkerStance) then
				Zorlen_WarriorSpam_SwapFromBerserkerToDoAction = nil;
			end
			if (Zorlen_WarriorSpam_CurrentStance == LOCALIZATION_ZORLEN_BattleStance) then
				Zorlen_WarriorSpam_SwapFromBattleToDoAction = nil;
			end
			if (Zorlen_WarriorSpam_CurrentStance == LOCALIZATION_ZORLEN_DefensiveStance) then
		...
		elseif (event == "CHAT_MSG_SPELL_SELF_DAMAGE") then
			if string.find(arg1, LOCALIZATION_ZORLEN_Taunt.." ") then
				Zorlen_debug("Target has resisted "..LOCALIZATION_ZORLEN_Taunt.."!");
				ZorlenWarriorSpamFrame.MockingBlowDelay_timer = nil;
				Zorlen_WarriorSpam_MockingBlowDelay = nil;
			end
...
function Zorlen_WarriorSpam_UseSelectedConfiguration(mode, tanking, rend, demo, pvp, charge, targ, disarm, taunt, aggrocharge, macro, test)
	...
	local CID = nil
	if Zorlen_Button[LOCALIZATION_ZORLEN_Rend] then
		if IsActionInRange(Zorlen_Button[LOCALIZATION_ZORLEN_Rend]) == 1 then
			CID = 1
		end
	else
	...
	if not test then
		if
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_Fear, "player") --Fear
			or
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_HowlOfTerror, "player") --Fear: Howl of Terror
			or
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_PsychicScream, "player") --Fear: Psychic Scream
			then
				FearEffect = 1;
				IncapacitateEffect = 1;
		elseif
			-- Rogue Stun effects
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_KidneyShot, "player")
			or
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_CheapShot, "player")
			or
			-- Warrior Stun Effect - to short to bother with?
			-- Warstomp - short stun not worth it?
			--Zorlen_checkDebuff("Ability_WarStomp", "player") or
			-- Paladin Stun Effect
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_HammerOfJustice, "player")
			then
				StunEffect = 1;
				IncapacitateEffect = 1;
		elseif
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_Polymorph, "player") --Polymorph
			or
			Zorlen_checkDebuff("Spell_Nature_Sleep", "player") --Sleep: Hibernate
			or
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_Sap, "player") --Sap
			or
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_Seduction, "player") --Succubus Seduction
			or
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_ScatterShot, "player") --Scatter Shot
			or
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_FreezingTrap, "player") --Freezing Trap
			or
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_Banish, "player") --Banish
			or
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_MindControl, "player") --Mind Control
			then
				IncapacitateEffect = 1;
		elseif
			-- Mage Immobilizing effect
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_FrostNova, "player")
			or
			-- Druid Root
		...
		elseif
			...
			or
			-- Rogue Slow effect
			Zorlen_checkDebuffByName(LOCALIZATION_ZORLEN_CripplingPoison, "player")
			or
	...
	if ActiveEnemy then
		if CID then
			if Zorlen_Button[LOCALIZATION_ZORLEN_HeroicStrike] then
				if (tanking == "yes") then
	...
	if Zorlen_Button[LOCALIZATION_ZORLEN_ShieldBash] and pvp ~= "yes" and TIETE and castShieldBash(test) then
		--castShieldBash()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_ShieldBash, macro, test)
	elseif Zorlen_Button[LOCALIZATION_ZORLEN_Pummel] and pvp ~= "yes" and TIETE and castPummel(test) then
		--castPummel()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Pummel, macro, test)
	elseif Zorlen_Button[LOCALIZATION_ZORLEN_ConcussionBlow] and pvp ~= "yes" and TIETE and castConcussionBlow(test) then
		--castConcussionBlow()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_ConcussionBlow, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_BerserkerRage] and Zorlen_Button[LOCALIZATION_ZORLEN_ConcussionBlow] and pvp ~= "yes" and TIETE and CID and isBerserkerStance() and PlayerMana < 15 and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_ConcussionBlow) and (BRRP + PlayerMana) >= 15 and castBerserkerRage(test)
		then
		--castBerserkerRage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BerserkerRage, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Bloodrage] and Zorlen_Button[LOCALIZATION_ZORLEN_ConcussionBlow] and pvp ~= "yes" and TIETE and CID and PlayerMana < 15 and PlayerHealthFraction >= 0.5 and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_ConcussionBlow) and (Zorlen_BloodrageRagePoints() + PlayerMana) >= 15 and castBloodrage(test)
		then
		--castBloodrage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Bloodrage, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_BerserkerRage] and Zorlen_Button[LOCALIZATION_ZORLEN_ShieldBash] and pvp ~= "yes" and TIETE and CID and Zorlen_isShieldEquipped() and isBerserkerStance() and PlayerMana < 10 and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_ShieldBash) and TMRP >= 10 and (BRRP + PlayerMana) >= 10 and castBerserkerRage(test)
		then
		--castBerserkerRage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BerserkerRage, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_ShieldBash] and pvp ~= "yes" and TIETE and CID and Zorlen_isShieldEquipped() and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_ShieldBash) and isBerserkerStance()
		 and
		(
			(
			 ...
			 or
			(
				PlayerHealthFraction >= 0.5 and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Bloodrage)
			)
		)
		 and
		...
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_DefensiveStance, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Pummel] and pvp ~= "yes" and not isBerserkerStance() and TIETE and CID and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Pummel)
		 and
		(
			(
				PlayerMana >= 10 and TMRP >= 10
			...
			(
			 ...
			 or
			(
				Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_BerserkerRage)
				 and
				(
					BRRP >= 10
					 or
	...
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_BerserkerRage] and Zorlen_Button[LOCALIZATION_ZORLEN_Pummel] and pvp ~= "yes" and TIETE and CID and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Pummel) and isBerserkerStance() and PlayerMana < 10
		 and
		(
			BRRP >= 10
			 or
		...
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BerserkerRage, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Bloodrage] and pvp ~= "yes" and TIETE and CID and PlayerMana < 10 and PlayerHealthFraction >= 0.5
		 and
		(
			(
				Zorlen_isShieldEquipped() and not isBerserkerStance() and Zorlen_Button[LOCALIZATION_ZORLEN_ShieldBash] and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_ShieldBash)
			)
			 or
			(
				Zorlen_Button[LOCALIZATION_ZORLEN_Pummel] and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Pummel) and isBerserkerStance()
			)
		)
		 and
		castBloodrage(test)
		...
		--castBloodrage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Bloodrage, macro, test)
	elseif Zorlen_Button[LOCALIZATION_ZORLEN_Taunt] and (tanking == "yes") and (taunt == "yes") and castTaunt(test) then
		if not test then
	...
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_MockingBlow] and (mode == "hate") and (taunt == "yes") and not Zorlen_WarriorSpam_MockingBlowDelay and not Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Taunt) and castMockingBlow(test)
		then
		--castMockingBlow()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_MockingBlow, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_BerserkerRage] and Zorlen_Button[LOCALIZATION_ZORLEN_MockingBlow] and (mode == "hate") and (taunt == "yes") and not Zorlen_WarriorSpam_MockingBlowDelay and Zorlen_TargetIsEnemyTargetingFriendButNotYou() and CID and not Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Taunt) and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_MockingBlow) and isBerserkerStance() and PlayerMana < 10 and TMRP >= 10 and (BRRP + PlayerMana) >= 10 and castBerserkerRage(test)
		then
		--castBerserkerRage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BerserkerRage, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_MockingBlow] and (mode == "hate") and (taunt == "yes") and not isBattleStance() and not Zorlen_WarriorSpam_MockingBlowDelay and Zorlen_TargetIsEnemyTargetingFriendButNotYou() and CID and not Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Taunt) and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_MockingBlow)
		 and
		(
			(
				PlayerMana >= 10 and TMRP >= 10
	...
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Bloodrage] and Zorlen_Button[LOCALIZATION_ZORLEN_MockingBlow] and (mode == "hate") and (taunt == "yes") and not Zorlen_WarriorSpam_MockingBlowDelay and Zorlen_TargetIsEnemyTargetingFriendButNotYou() and CID and not Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Taunt) and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_MockingBlow) and isBattleStance() and PlayerMana < 10 and PlayerHealthFraction >= 0.5 and castBloodrage(test)
		then
		--castBloodrage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Bloodrage, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Charge] and not ImmobilizingEffect and not IncapacitateEffect and not IsAltKeyDown()
		 and
		(
			(
		...
		then
		if not test then
			Zorlen_SetTimer(5, "CastChargeDelay", nil, "InternalZorlenMiscTimer", 2, 1)
		end
		--castCharge()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Charge, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Intercept] and not ImmobilizingEffect and not IncapacitateEffect and not IsAltKeyDown()
		 and
		(
			(
				(aggrocharge == "yes") and ActiveEnemy
	...
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Charge] and not ImmobilizingEffect and not IncapacitateEffect and not isBattleStance() and not IsAltKeyDown()
		 and
		(
			(
				(aggrocharge == "yes") and ActiveEnemy
		 ...
		 and
		IsActionInRange(Zorlen_Button[LOCALIZATION_ZORLEN_Charge]) == 1 and Zorlen_notInCombat() and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Charge)
		 and
		castBattleStance(test)
		then
		if not test and tanking ~= "yes" and not Zorlen_WarriorSpam_SwapFromBerserkerToDoAction and not Zorlen_WarriorSpam_SwapFromDefensiveToDoAction and not Zorlen_WarriorSpam_SwapFromBattleToDoAction then
		...
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BattleStance, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Intercept] and not ImmobilizingEffect and not IncapacitateEffect and not isBerserkerStance() and not IsAltKeyDown()
		 and
		...
		(
		 ...
		 and
		((Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Charge) and Zorlen_Combat) or not Zorlen_IsTimer("CastChargeDelay", nil, "InternalZorlenMiscTimer"))
		 and
		(IsActionInRange(Zorlen_Button[LOCALIZATION_ZORLEN_Intercept]) == 1) and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Intercept)
		 and
		(
			(
				PlayerMana >= 10 and TMRP >= 10
		...
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BerserkerStance, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_BerserkerRage] and Zorlen_Button[LOCALIZATION_ZORLEN_Intercept] and not ImmobilizingEffect and not IncapacitateEffect
		 and
		(
			(
				(aggrocharge == "yes") and ActiveEnemy
		...
		(
			(
				Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Charge) and Zorlen_Combat
			)
			 or
			not Zorlen_IsTimer("CastChargeDelay", nil, "InternalZorlenMiscTimer")
		)
		and
		(IsActionInRange(Zorlen_Button[LOCALIZATION_ZORLEN_Intercept]) == 1) and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Intercept) and PlayerMana < 10 and (BRRP + PlayerMana) >= 10 and castBerserkerRage(test)
		then
		--castBerserkerRage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BerserkerRage, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Bloodrage] and Zorlen_Button[LOCALIZATION_ZORLEN_Intercept] and not ImmobilizingEffect and not IncapacitateEffect
		 and
		(
			(
				(aggrocharge == "yes") and ActiveEnemy
		...
		(
		 ...
		 and
		(IsActionInRange(Zorlen_Button[LOCALIZATION_ZORLEN_Intercept]) == 1) and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Intercept) and PlayerMana < 10 and castBloodrage(test)
		then
		--castBloodrage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Bloodrage, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Hamstring] and not boss
		 and
		(
			(pvp == "yes")
			 or
	...
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_BerserkerRage] and Zorlen_Button[LOCALIZATION_ZORLEN_Hamstring] and not boss
		 and
		(
			not Zorlen_WasHamstringSpellCastImmune
			 or
			(
				Zorlen_WasHamstring and not Zorlen_IsTimer(LOCALIZATION_ZORLEN_Hamstring, "immune", "InternalZorlenMiscTimer")
			)
			 or
			(
				(pvp == "yes") and not Zorlen_IsTimer(LOCALIZATION_ZORLEN_Hamstring, "immune", "InternalZorlenMiscTimer")
			)
		)
		 and
		not hamstrung
		...
		(
		 ...
		 and
		CID and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Hamstring) and isBerserkerStance() and PlayerMana < 10 and (BRRP + PlayerMana) >= 10 and castBerserkerRage(test)
		then
		--castBerserkerRage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BerserkerRage, macro, test)
	elseif
		...
		(
		 ...
		 and
		CID and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Hamstring) and isDefensiveStance()
		 and
		(
			(
				PlayerMana >= 10 and TMRP >= 10
	...
	elseif
		...
		(
			(
				PlayerMana < 10 and (pvp == "yes")
				 and
				(
					not Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Bloodrage) or PlayerHealthFraction < 0.5
				)
			)
			 or
			(
		 ...
		 and
		CID and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Hamstring) and not isBerserkerStance()
		 and
		(
			(
			...
			(
				...
				(
					(
						Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Bloodrage) and PlayerHealthFraction >= 0.5 and not isBattleStance()
					)
					 or
					(
						Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_BerserkerRage)
		...
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BerserkerStance, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Bloodrage] and Zorlen_Button[LOCALIZATION_ZORLEN_Hamstring] and not boss
		 and
		(
			not Zorlen_WasHamstringSpellCastImmune
			 or
		...
		(
		 ...
		 and
		CID and PlayerMana < 10 and PlayerHealthFraction >= 0.5 and not isDefensiveStance() and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Hamstring) and castBloodrage(test)
		then
		--castBloodrage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Bloodrage, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Revenge]
		 and
		(
			mode == "hate"
			 or
			(
				(
					tanking ~= "yes"
					 or
					(
						Zorlen_IsTimer(LOCALIZATION_ZORLEN_SunderArmor, "immune", "InternalZorlenMiscTimer")
						 or
						(
							(
								Zorlen_GetTimer(LOCALIZATION_ZORLEN_SunderArmor, nil, "InternalZorlenSpellTimers") > Zorlen_WarriorSpam_SunderRemainingDurationCastTime
							)
							 or
							not isSunder()
						)
				...
				(
					...
					(
					 ...
					 and
					CID and Zorlen_Button[LOCALIZATION_ZORLEN_Hamstring] and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Hamstring)
					 and
					(
						TMRP >= 10
						 or
						(
							PlayerMana < 10 and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Bloodrage) and PlayerHealthFraction >= 0.5
						)
						 or
						(
							PlayerMana < 10 and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_BerserkerRage)
							 and
							(
								BRRP >= 10
								 or
	...
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Overpower] and tanking ~= "yes"
		 and
		not
		(
			not boss
	...
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_BerserkerRage] and Zorlen_Button[LOCALIZATION_ZORLEN_Overpower] and tanking ~= "yes" and CID and Zorlen_IsTimer("TargetDodgedYou_Overpower", nil, "InternalZorlenMiscTimer") and Zorlen_GetTalentRank(LOCALIZATION_ZORLEN_ImprovedOverpower) >= 1 and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Overpower) and isBerserkerStance() and PlayerMana < 5 and TMRP >= 5 and BRRP >= 5 and castBerserkerRage(test)
		then
		--castBerserkerRage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BerserkerRage, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Bloodrage] and Zorlen_Button[LOCALIZATION_ZORLEN_Overpower] and tanking ~= "yes" and CID and Zorlen_IsTimer("TargetDodgedYou_Overpower", nil, "InternalZorlenMiscTimer") and ActiveEnemy and Zorlen_GetTalentRank(LOCALIZATION_ZORLEN_ImprovedOverpower) >= 1 and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Overpower) and isBattleStance() and PlayerMana < 5 and PlayerHealthFraction >= 0.5 and castBloodrage(test)
		then
		--castBloodrage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Bloodrage, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Overpower] and tanking ~= "yes" and Zorlen_IsTimer("TargetDodgedYou_Overpower", nil, "InternalZorlenMiscTimer") and not isBattleStance() and CID and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Overpower)
		 and
		(
		...
		(
			(
			 ...
			 or
			(
				Zorlen_GetTalentRank(LOCALIZATION_ZORLEN_ImprovedOverpower) >= 1
				 and
				(
					(
						PlayerMana <= (TMRP + 10)
		...
		--castBattleStance()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BattleStance, macro, test)
	elseif Zorlen_Button[LOCALIZATION_ZORLEN_Execute] and tanking ~= "yes" and castExecute(test) then
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Execute, macro, test)
	elseif
		Zorlen_Button[LOCALIZATION_ZORLEN_Bloodrage] and (mode == "hate") and ActiveEnemy and CheckInteractDistance("target", 3) and PlayerHealthFraction > 0.5 and castBloodrage(test)
		then
		--castBloodrage()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Bloodrage, macro, test)
	elseif Zorlen_Button[LOCALIZATION_ZORLEN_ShieldBash] and (mode == "hate") and (((Zorlen_GetTimer(LOCALIZATION_ZORLEN_SunderArmor, nil, "InternalZorlenSpellTimers") > Zorlen_WarriorSpam_SunderRemainingDurationCastTime or not Zorlen_Button[LOCALIZATION_ZORLEN_SunderArmor]) and PlayerMana >= (15 + Zorlen_ShieldSlamRageCost())) or PlayerMana >= (10 + Zorlen_SunderArmorRageCost() + Zorlen_ShieldSlamRageCost())) and isSunder() and (not Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_ShieldSlam) or not Zorlen_isShieldEquipped()) and castShieldBash(test) then
		--castShieldBash()
		Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_ShieldBash, macro, test)
	elseif (tanking == "yes") then
		if
			CID and PlayerMana < 35
			 and
			(
				mode == "hate" or Zorlen_IsTimer(LOCALIZATION_ZORLEN_Hamstring, "immune", "InternalZorlenMiscTimer")
				 or
				(
			...
			Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_DefensiveStance, macro, test)
		elseif
			Zorlen_Button[LOCALIZATION_ZORLEN_ShieldSlam]
			 and
			(
				(
					(
						not Zorlen_Button[LOCALIZATION_ZORLEN_SunderArmor]
						 or
						(
							(
		...
		elseif
			Zorlen_Button[LOCALIZATION_ZORLEN_ShieldBlock] and ((Zorlen_GetTimer(LOCALIZATION_ZORLEN_SunderArmor, nil, "InternalZorlenSpellTimers") > Zorlen_WarriorSpam_SunderRemainingDurationCastTime and PlayerMana >= 15) or PlayerMana >= (15 + Zorlen_SunderArmorRageCost())) and isSunder() and (mode ~= "hate" or not Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_ShieldSlam) or not Zorlen_isShieldEquipped()) and Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_Revenge)
			 and
		...
		elseif
			Zorlen_Button[LOCALIZATION_ZORLEN_Disarm] and mode ~= "hate" and (disarm == "yes")
			 and
			(
				Zorlen_IsTimer(LOCALIZATION_ZORLEN_SunderArmor, "immune", "InternalZorlenMiscTimer")
				 or
				(
					Zorlen_GetTimer(LOCALIZATION_ZORLEN_SunderArmor, nil, "InternalZorlenSpellTimers") > Zorlen_WarriorSpam_SunderRemainingDurationCastTime and isSunder()
				)
			)
			 and
			castDisarm(test)
		...
		elseif
			Zorlen_Button[LOCALIZATION_ZORLEN_DemoralizingShout] and mode ~= "hate" and (demo == "yes")
			 and
			(
				Zorlen_IsTimer(LOCALIZATION_ZORLEN_SunderArmor, "immune", "InternalZorlenMiscTimer")
				 or
		...
		elseif
			Zorlen_Button[LOCALIZATION_ZORLEN_BattleShout] and mode ~= "hate"
			 and
			(
				Zorlen_IsTimer(LOCALIZATION_ZORLEN_SunderArmor, "immune", "InternalZorlenMiscTimer")
				 or
			...
			Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BattleShout, macro, test)
		elseif 
			Zorlen_Button[LOCALIZATION_ZORLEN_Rend] and mode ~= "hate" and (rend == "yes")
			 and
			(
				Zorlen_IsTimer(LOCALIZATION_ZORLEN_SunderArmor, "immune", "InternalZorlenMiscTimer")
				 or
				(
					Zorlen_GetTimer(LOCALIZATION_ZORLEN_SunderArmor, nil, "InternalZorlenSpellTimers") > Zorlen_WarriorSpam_SunderRemainingDurationCastTime and isSunderFull()
				)
			)
			 and
			((not Zorlen_Button[LOCALIZATION_ZORLEN_SunderArmor] and PlayerMana >= 15) or (PlayerMana >= (10 + Zorlen_SunderArmorRageCost())))
			 and
			castRend(test)
			then
			--castRend()
		...
		elseif
			Zorlen_Button[LOCALIZATION_ZORLEN_SunderArmor]
			 and
			(
				(
					(
				 ...
				 or
				(
					(not Zorlen_Button[LOCALIZATION_ZORLEN_ShieldSlam] or not Zorlen_isShieldEquipped() or mode ~= "hate" or not Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN_ShieldSlam))
					 and
					not
					(
						(
		...
		elseif Zorlen_Button[LOCALIZATION_ZORLEN_SunderArmor] and ZorlenWarriorSpamConfig[ZPN][UNLIMITEDSUNDER] then
			if PlayerMana >= (25 + Zorlen_SunderArmorRageCost()) or ((not Zorlen_Button[LOCALIZATION_ZORLEN_ShieldSlam] or not Zorlen_isShieldEquipped()) and (PlayerMana >= (10 + Zorlen_SunderArmorRageCost()))) then
		...
		if Zorlen_Button[LOCALIZATION_ZORLEN_Disarm] and (disarm == "yes") and not Zorlen_TargetIsDieingEnemy() and castDisarm(test) then
			--castDisarm()
			Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Disarm, macro, test)
		elseif Zorlen_Button[LOCALIZATION_ZORLEN_BattleShout] and castBattleShout(test) then
			--castBattleShout()
			Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_BattleShout, macro, test)
		elseif Zorlen_Button[LOCALIZATION_ZORLEN_Rend] and (rend == "yes") and not Zorlen_TargetIsDieingEnemy() and castRend(test) then
			--castRend()
			Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_Rend, macro, test)
		elseif Zorlen_Button[LOCALIZATION_ZORLEN_DemoralizingShout] and (demo == "yes") and not Zorlen_TargetIsDieingEnemy() and castDemoralizingShout(test) then
			--castDemoralizingShout()
			Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_DemoralizingShout, macro, test)
		elseif
			Zorlen_Button[LOCALIZATION_ZORLEN_MortalStrike]
			 and 
			(
				(
					(isDefensiveStance() or not Zorlen_Button[LOCALIZATION_ZORLEN_Execute]) and PlayerMana >= 35
				)
				 or
		...
		elseif
			Zorlen_Button[LOCALIZATION_ZORLEN_Bloodthirst]
			 and 
			(
				(
					(isDefensiveStance() or not Zorlen_Button[LOCALIZATION_ZORLEN_Execute]) and PlayerMana >= 35
		...
		elseif
			...
			(
				(
					(isDefensiveStance() or not Zorlen_Button[LOCALIZATION_ZORLEN_Execute]) and PlayerMana >= 25
				)
				 or
			...
			--castShieldSlam()
			Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_ShieldSlam, macro, test)
		elseif Zorlen_Button[LOCALIZATION_ZORLEN_ShieldBlock] and PlayerMana >= 35 and castShieldBlock(test) then
			--castShieldBlock()
			Zorlen_WarriorSpam_UpdateIcons(Zorlen_WarriorSpam_IconIndex_ShieldBlock, macro, test)
		elseif
			...
			(
				...
				not
				(
					(rend == "yes") and not isRend() and not Zorlen_IsTimer(LOCALIZATION_ZORLEN_Rend, "immune", "InternalZorlenMiscTimer")
				)
			)
			 and
			(
			 ...
			 and
			(
				isDefensiveStance() or Zorlen_IsTimer(LOCALIZATION_ZORLEN_Hamstring, "immune", "InternalZorlenMiscTimer")
				 or
				(
					Zorlen_WasHamstringSpellCastImmune and not Zorlen_WasHamstring
				)
...
function Zorlen_WarriorSpam_TestButtons()
	...
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Execute] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Execute) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Execute] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Execute.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_HeroicStrike] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_HeroicStrike) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_HeroicStrike] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_HeroicStrike.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_SunderArmor] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_SunderArmor) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_SunderArmor] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_SunderArmor.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_MortalStrike] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_MortalStrike) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_MortalStrike] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_MortalStrike.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Bloodthirst] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Bloodthirst) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Bloodthirst] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Bloodthirst.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_ShieldSlam] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_ShieldSlam) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_ShieldSlam] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_ShieldSlam.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Charge] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Charge) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Charge] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Charge.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Taunt] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Taunt) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Taunt] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Taunt.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Intercept] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Intercept) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Intercept] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Intercept.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Overpower] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Overpower) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Overpower] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Overpower.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Revenge] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Revenge) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Revenge] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Revenge.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Rend] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Rend) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Rend] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Rend.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Hamstring] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Hamstring) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Hamstring] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Hamstring.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_ShieldBash] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_ShieldBash) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_ShieldBash] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_ShieldBash.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Pummel] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Pummel) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Pummel] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Pummel.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_ShieldBlock] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_ShieldBlock) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_ShieldBlock] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_ShieldBlock.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_DemoralizingShout] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_DemoralizingShout) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_DemoralizingShout] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_DemoralizingShout.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_BattleShout] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_BattleShout) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_BattleShout] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_BattleShout.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_BerserkerRage] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_BerserkerRage) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_BerserkerRage] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_BerserkerRage.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Bloodrage] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Bloodrage) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Bloodrage] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Bloodrage.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_DeathWish] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_DeathWish) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_DeathWish] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_DeathWish.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_MockingBlow] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_MockingBlow) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_MockingBlow] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_MockingBlow.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Disarm] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_Disarm) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_Disarm] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Disarm.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_ConcussionBlow] then
		if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN_ConcussionBlow) then
			Zorlen_RegisterButtons()
			if not Zorlen_Button[LOCALIZATION_ZORLEN_ConcussionBlow] then
				Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_ConcussionBlow.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
				return
			end
		end
	end
	if not Zorlen_Button[LOCALIZATION_ZORLEN_Attack] then
		Zorlen_RegisterButtons()
		if not Zorlen_Button[LOCALIZATION_ZORLEN_Attack] then
			Zorlen_debug("You must put "..LOCALIZATION_ZORLEN_Attack.." on one of your action bars (even if it is hidden) for Zorlen WarriorSpam to work right!", 1);
			return
		end
	end
end
