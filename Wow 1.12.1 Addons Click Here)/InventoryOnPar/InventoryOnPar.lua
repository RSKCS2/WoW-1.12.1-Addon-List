...
local INVENTORYONPAR_VERSION = 3.54;
...
ITEM_RARITY = {};
ITEM_RARITY[0]={ name=POOR, colour=colourGrey };
ITEM_RARITY[1]={ name=COMMON, colour=colourWhite };
ITEM_RARITY[2]={ name=UNCOMMON, colour=colourGreen };
ITEM_RARITY[3]={ name=RARE, colour=colourDarkBlue };
ITEM_RARITY[4]={ name=EPIC, colour=colourPurple };
ITEM_RARITY[5]={ name=LEGENDARY, colour=colourOrange };
ITEM_RARITY[6]={ name=ARTIFACT, colour=colourGold };
	...
	for index = 1, getn(INVENTORY_SLOT_LIST), 1 do
		...
		if (slotLink ~= nil) then
			...
			if (itemCode) then
				...
				if(levelOK) then
					local itemScore = itemLevel - unitLevel;
					if(itemRarity>2) then  -- we have a rare or better item so add a bonus
						itemScore = itemScore + 5 * (itemRarity - 2);
					end
					if(itemSubType== TH_MACE or itemSubType == TH_SWORD or itemSubType == TH_AXE or itemSubType == STAVES or itemSubType == POLEARMS) then
			...
			if(INVENTORY_SLOT_LIST[index].name == "SecondaryHandSlot" and iopTwoHanded == "true") then
				...
				iopValue = 0;
			elseif(IOP_TooLowLevel(index, unitLevel, UnitClass(unitName))) then
				iopValue = 0;
			else
				iopValue = - unitLevel * INVENTORY_SLOT_LIST[index].weight * 1.5;
				iopTotal = iopTotal + iopValue;
			end
			textstring=textstring.."  ("..format("%.2f", iopValue)..")";
		end
...
function IOP_CheckLevel(itemLevel)
	...
	elseif (type(itemLevel) == "string") then
		if(itemLevle =="0" or itemLevel == "1") then
			return nil;
		end
	end
	return "itemok";
...
function IOP_TooLowLevel(index, unitLevel, unitClass)
	...
	if (INVENTORY_SLOT_LIST[index].name == "RangedSlot" and unitLevel < 52) then
		-- paladins, shamans & druids need to be level 52 to use Libram, Totem or Idol
