MetaHud_variables = {
	...
	["<pvp>"] = { 
		func = function(text, unit)
			if (UnitIsPVPFreeForAll(unit) or UnitIsPVP(unit)) and not MetaHud:TargetIsNPC() then
				text = MetaHud:gsub(text, '<pvp>', "PVP");
			else
				text = MetaHud:gsub(text, '<pvp>', "");
			end
			return text;
		end,
	...
	["<range>"] = { 
		func = function(text, unit)
			...
			if(this.onTaxi ~= nil) then
				local eta = string.format("%d:%02d", (this.endTime - GetTime()) / 60, math.mod((this.endTime - GetTime()), 60))
				local dur = string.format("%d:%02d", this.onTaxi / 60, math.mod(this.onTaxi, 60))
				r = "|cff00ffff"..FlightMapTimesFrame.endPoint..": "..dur.."  [|r"..eta.."|cff00ffff]";
	...
	["<totarget>"] = { 
		func = function(text, unit)
	      ...
	      MetaHud_ToTargetHealth_Bar:SetValue(UnitHealth("targettarget"));
				getglobal("MetaHud_ToTarget_Text"):EnableMouse(1);
				...
				getglobal("MetaHud_ToTarget_Text"):EnableMouse(0);
